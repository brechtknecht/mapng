...
// --- Corridor masking (route mode) ------------------------------------------
// Drop mesh triangles whose XZ centroid lies more than halfWidthM (perpendicular)
// from the route polyline, then COMPACT the geometry so the dropped vertices
// actually leave the file (an index-only trim keeps the big position buffers).
// Operates in the shared scene XZ frame produced by latLngToScene — the frame
// the Google group's geometry is already in.
...
const clipGroupToCorridorXZ = (group, data, segment, halfWidthM, onProgress) => {
  ...
  const halfScene = halfWidthM * _cachedUnitsPerMeter;
